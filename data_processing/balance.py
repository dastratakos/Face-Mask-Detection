...
from config import BALANCED_IMAGE_ROOT, CROPPED_BY_CLASS_ROOT, FORMAT
...
def upsample(className, target):
    os.makedirs(BALANCED_IMAGE_ROOT + className, exist_ok=True)
    filenames = os.listdir(CROPPED_BY_CLASS_ROOT + className)
    print(f'Upsampling {className} from {len(filenames)} to {target}')
    with tqdm(total=target) as progress_bar:
        # copy original images
        for filename in filenames:
            shutil.copy(CROPPED_BY_CLASS_ROOT + f'{className}/' + filename,
                        BALANCED_IMAGE_ROOT + className)
            progress_bar.update()
        # augment the rest
        for i in range(target - len(filenames)):
            filename = random.choice(filenames)
            im = io.imread(CROPPED_BY_CLASS_ROOT + f'{className}/' + filename)
            im2 = augment.augment_image(im)
            io.imsave(BALANCED_IMAGE_ROOT + \
...
def printDistribution():
    num_no_mask = len(os.listdir(CROPPED_BY_CLASS_ROOT + 'no_mask'))
    num_mask = len(os.listdir(CROPPED_BY_CLASS_ROOT + 'mask'))
    num_incorrect = len(os.listdir(CROPPED_BY_CLASS_ROOT + 'incorrect'))
...
def main():
    ...
    logging.info('========== Balancing module ==========')
    os.makedirs(BALANCED_IMAGE_ROOT, exist_ok=True)
